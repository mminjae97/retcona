...
def check_nickname_column_length(connection: Connection) -> None:
    ...
    """
    # Only a missing table/column is caught, not errors from inside reflection
    # itself, which would otherwise be misreported as a migration problem.
    # check_schema_is_current runs first, so this mostly matters when it was
    # skipped (no db/migrations) or DATABASE_URL points at the wrong database.
    try:
        reflected = inspect(connection).get_columns("users")
    except NoSuchTableError:
        reflected = []
    nickname_column = next((c for c in reflected if c["name"] == "nickname"), None)
    ...
    # a narrowing ALTER, so it's logged.
    ...
    if actual_length != expected_length:
        logger.warning(
            ...
            f"VARCHAR({actual_length})" if actual_length is not None else "unbounded",
            expected_length,
        )
