...
1. Claim extraction (`pipeline/extract_claims.py`). The model is given the novel's character and location names — each character with the aliases the author listed on its card (nicknames, titles) — and names each claim's subject by the listed name; one it still gives as an alias is renamed after its character. Within a novel no two characters share a name or alias (409 on the settings screen), and adding an alias to a card moves what was recorded under that name (dismissals, unlinked claims) to the card, as renaming does.
