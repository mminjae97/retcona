...
`python -m workers.purge --loop` retries the startup check every minute while the database can't be reached, for up to 10 minutes (then it exits non-zero: a wrong password or host looks the same as a database still starting — so run it under a restart policy). A pass stops early only when the database itself fails (unreachable or lost partway, shutting down, read-only after a failover, out of disk, I/O errors or corruption, a table renamed by a newer migration: `_DATABASE_WIDE_SQLSTATES`), logging in one ERROR how many accounts it had already purged. Any other failure is counted against its account and the pass goes on, so no account can keep the ones after it from being purged; if every account a pass attempted failed with the same SQLSTATE, that's logged as an ERROR too (a cause they share). Either kind of failed pass is retried after an hour, up to 3 times in a row, then at the next midnight; accounts that failed alongside ones that didn't are logged as a warning and simply wait for the next midnight.
