...
1. Claim extraction (`pipeline/extract_claims.py`). The model is given the novel's characters — each with a ref, its name and the aliases the author listed on its card (nicknames, titles) — and location names, and answers a claim about a known character with its ref. Two characters can share a name or an alias but not both: characters with one name need aliases, none shared (409 on the settings screen), and the model tells them apart by the context and those. A claim with no usable ref is matched by name, then alias (`pipeline/entities.py`); a name that fits more than one character leaves the claim unlinked — not judged, and no new card made for it.
...
The LLMs aren't chosen yet (chapter 5). There are two, each set on its own: `LLM_MODEL_EXTRACTION` for claim extraction (most of the calls; a fast, cheaper model) and `LLM_MODEL_JUDGMENT` for judgments — OOC, the final check of contradictions NLI finds ambiguous (a stronger model); `infra/llm_client.py`. Until then `LLM_PROVIDER=mock` (`infra/mock_llm.py`) answers the extraction prompt from keyword rules, rough but enough to run everything end to end. Failures are recorded on the run as a code: `queue_unavailable`, `abandoned`, `episode_missing`, `empty_manuscript`, `llm_failed`, `bad_llm_response`, `inference_failed` (the NLI model couldn't be downloaded, loaded or run), `internal`.
