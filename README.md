...
#### 6.1 계정 관리 · 마이페이지
이메일 또는 구글 계정으로 가입하고, 실명 대신 필명(닉네임)을 정해 활동한다. 마이페이지에서 여러 작품을 한 계정으로 등록·관리하고, 작품별 인물 관계도·타임라인을 확인하며, 필요 시 회원 탈퇴까지 처리할 수 있다.
...
| 방식 | 제공자 | 비고 |
|---|---|---|
| 자체 로그인 | 이메일 + 비밀번호 | 비밀번호는 해시(bcrypt 등)로 저장. 가입 폼에 필명(닉네임) 입력란 포함. 가입 전에 이메일로 보낸 6자리 인증번호로 주소 소유를 확인 |
| 소셜 로그인 | Google | OAuth 2.0 (인가 코드 + PKCE), 최초 로그인 시 필명 설정 화면에서 필명을 정하면 계정 생성(3.2 참고) |

#### 3.2 소셜 로그인 흐름 (Google)
...
sequenceDiagram
    ...
    U->>FE: 구글 로그인 버튼 클릭
    FE->>G: 인가 요청 (state, PKCE code_challenge)
    G-->>FE: 인가 코드 반환 (콜백 페이지)
    FE->>API: 인가 코드 + code_verifier 전달
    API->>G: 토큰 교환 요청 (client secret은 서버만 보유)
    G-->>API: ID 토큰 (구글 계정 ID, 인증된 이메일)
    API->>DB: 구글 계정 ID로 사용자 조회
    alt 기존 계정
        ...
        U->>FE: 필명 입력
        FE->>API: 가입 대기 토큰 + 필명
        API->>DB: 사용자 생성 (provider=google)
        API-->>FE: 인증 토큰 발급 (JWE로 암호화)
    end
```

- 계정은 필명을 정한 뒤에 만들어진다 — 필명 없는 계정이 존재하지 않도록(`users.nickname`은 필수)
- 같은 이메일이 이미 이메일·비밀번호로 가입돼 있으면 연결하지 않고 거부한다 — 이메일 인증 도입(3.1) 이전에 가입한 계정은 주소 소유를 확인하지 않았으므로, 이메일로 자동 연결하면 남의 Gmail 주소로 먼저 가입해 둔 사람이 그 계정에 접근할 수 있게 된다
- 탈퇴 유예 중인 계정의 구글 로그인은 이메일 로그인과 똑같이 탈퇴를 취소한다(3.5)
...
- [x] **1단계** — 로그인/소셜 로그인 + 마이페이지(작품 관리·회원 탈퇴) + 원고 작성 에디터(임시저장/저장) + 화 단위 엔티티 추출, 멀티테넌시 반영한 설정 카드 스키마 설계
- [ ] **2단계** — 외형 불일치 + 장소 모순 탐지 (NLI 기반)
- [ ] **3단계** — 설정 보완 후 재검증 (플래그 단위 재판단)
- [ ] **4단계** — 사건 타임라인 대조 (상태 이력 누적 로직)
- [ ] **5단계** — OOC 행동 탐지 (LLM 기반)
...
출시 전 마무리 단계에서 처리할 항목(구글 로그인 운영용 OAuth 클라이언트 교체, 인증번호 메일 실제 발송 설정, NLI 모델 배포 위치와 학습 데이터 출처 표기)은 [docs/pre-launch-checklist.md](docs/pre-launch-checklist.md)에 정리되어 있습니다.
